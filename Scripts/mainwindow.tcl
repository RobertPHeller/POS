...
#
  ...
  delegate option -countvariable to itemcount as -textvariable
  delegate option -grandtotalvariable to grandtotal as -textvariable
...
#    puts stderr "*** ${type}::constructor: wm state $toplevel = [wm state $toplevel]"
    ...
    label $frame.himg -image [IconImage image DeepwoodsBanner]
    pack $frame.himg -expand yes -fill x
    ...
    set header [frame $main.header]
    pack $header -expand yes -fill x
    grid [ttk::label $header.index -text "No." -anchor w] -row 0 -column 0 -sticky news
    grid [ttk::label $header.qnty -text "Qty" -anchor e] -row 0 -column 1 -sticky news
